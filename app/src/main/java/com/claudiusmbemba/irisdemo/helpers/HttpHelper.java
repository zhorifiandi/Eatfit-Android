...
public class HttpHelper {
    ...
    public static String makeRequest(RequestPackage requestPackage, InputStream data)
        ...
        if (requestPackage.getMethod().equals("POST")) {
            ...
            if (iris) {
                //TODO: CHANGE ME!!
                requestBuilder.addHeader("Prediction-Key","54e52158f13447e2bb1290d3dd415b7b");
...
//                        requestBody = RequestBodyUtil.create(MediaType.parse("application/octet-stream; charset=utf-8"), data);
                        requestBody = RequestBodyUtil.create(MediaType.parse("multipart/form-data; charset=utf-8"), data);
                    } else {
        ...
        if (response.isSuccessful()) {
            Log.d("COBA",response.body().string());
            return response.body().string();
        } else {
            throw new IOException("Exception: response code " + response.code() +" " + response.message());
        }
    }
}
